...
public class SootUtils {
     ...
     * @param entryMethodSig the entry method signature
     */
    public static void setEntryPoints(String tgtClass, String... entryMethodSig) {
        // 设置入口方法，默认入口方法是psvm，如果没有main方法的话则会出现找不到入口报错
        try {
            List<SootMethod> entryPoints = new ArrayList<>();
            if (entryMethodSig.length == 1) {
                System.out.printf("Add %d method in %s as entrypoint!\n", entryMethodSig.length, tgtClass);
            } else {
    ...
    public static void reportSootApplicationClassInfo() {
        ...
        for (SootClass sc : Scene.v().getApplicationClasses()) {
            ...
            int methodIndex = 0;
            System.out.printf(
                    "[%d] The class %s is an application class, loaded with %d methods! Methods are as followed:%n",
                    classIndex, sc.getName(), sc.getMethodCount());
